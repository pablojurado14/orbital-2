/**
 * Smoke test del adapter del clean core (Sesión 18 + 18.5 + 19.6).
 *
 ...
 *
 * Sesión 19.6 — cambios respecto a Sesión 18.5:
 *   - El adapter ahora recibe tx + clinicId explícitos (RLS multi-tenant).
 *   - El script abre withClinic(1, ...) y pasa tx a buildContextsFromDb /
 *     processEvent dentro del callback. Cierra deuda
 *     SMOKE-TEST-ADAPTER-OUT-OF-SYNC-V1.
 *
...
async function main() {
  ...
  await withClinic(clinicId, async (tx) => {
    ...
    const { state, runtimes, contexts, coordinatorOptions } =
      await buildContextsFromDb(tx, clinicId);
    ...
    console.log("\n--- Test 2: processEvent(proactive_tick) ---");
    const tickDecision = await processEvent(tx, clinicId, {
      kind: "proactive_tick",
      instant: Date.now(),
    ...
    const cancelDecision = await processEvent(tx, clinicId, {
      kind: "cancellation",
      instant: Date.now(),
      tenantId: String(clinicId),
